...
export default function Leadership() {
  ...
  const renderAvatarPlaceholder = (seed: string) => {
    let accentGradient = 'from-[#4C8DFF] to-white';
    let centerIcon = <Star className="w-6 h-6 text-[#4C8DFF] relative z-10" />;

    if (seed === 'CEO') {
      accentGradient = 'from-[#4C8DFF] via-white to-[#4C8DFF]';
      centerIcon = <Compass className="w-6 h-6 text-[#4C8DFF] relative z-10" />;
    } else if (seed === 'COO') {
      accentGradient = 'from-[#4C8DFF] to-[#A7B0C0]';
      centerIcon = <Network className="w-6 h-6 text-[#4C8DFF] relative z-10" />;
    } else if (seed === 'CTO') {
      accentGradient = 'from-[#4C8DFF] to-blue-300';
      centerIcon = <Code className="w-6 h-6 text-[#4C8DFF] relative z-10" />;
    } else if (seed === 'CIO') {
      accentGradient = 'from-emerald-400 to-[#4C8DFF]';
      centerIcon = <Shield className="w-6 h-6 text-[#4C8DFF] relative z-10" />;
    } else if (seed === 'CMO') {
      accentGradient = 'from-[#4C8DFF] to-pink-400';
      centerIcon = <Megaphone className="w-6 h-6 text-[#4C8DFF] relative z-10" />;
    }

    return (
      <div className="relative w-32 h-32 mx-auto rounded-full flex items-center justify-center p-[2px] bg-gradient-to-tr from-[#4C8DFF]/20 via-white/5 to-[#4C8DFF]/20 group-hover:from-[#4C8DFF]/60 group-hover:to-white/40 transition-all duration-300 overflow-hidden shadow-inner">
        {/* Decorative Radar Spinning Circle */}
        <div className="absolute inset-0 border border-[#4C8DFF]/10 rounded-full animate-spin-slow pointer-events-none" />
        <div className="absolute inset-2 border border-dashed border-[#4C8DFF]/15 rounded-full pointer-events-none" />
        
        {/* Gradient backing */}
        <div className="absolute inset-1 rounded-full bg-[#09111D] flex items-center justify-center">
          <div className="w-24 h-24 rounded-full bg-gradient-to-br from-[#15233A] to-[#0B1220] flex items-center justify-center shadow-lg relative group-hover:scale-95 transition-transform duration-300">
            {centerIcon}
            ...
            <span className="absolute bottom-1 right-1 px-1.5 py-0.5 rounded bg-[#09111D] border border-[#4C8DFF]/30 text-[8px] font-mono font-bold text-[#4C8DFF]">
              {seed}
            </span>
          </div>
        </div>
  ...
  return (
    <section id="leadership" className="relative py-28 bg-[#09111D]">
