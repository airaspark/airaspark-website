...
function mapCustomerDoc(id: string, data: Record<string, unknown>): Customer {
  return {
    id,
...
export async function getCustomerByCustomerId(
  ...
  return mapCustomerDoc(docSnap.id, docSnap.data());
}
...
export async function linkCustomerToFirebase(
  ...
  await updateDoc(doc(db, COLLECTIONS.customers, customerDocId), {
    firebaseUid,
...
export function isValidCustomerIdFormat(customerId: string): boolean {
  return /^ASC-\d{4}-\d{3}$/i.test(customerId.trim());
}
...
export async function getCustomers(): Promise<Customer[]> {
  ...
  return snapshot.docs.map((docSnap) =>
    mapCustomerDoc(docSnap.id, docSnap.data())
  );
}
